...
def get_measures(_pos, _neg, recall_level=recall_level_default):
    ...
    # 以防万一，删除其中的Nan 以及 INF
    pos=np.delete(pos, np.where(np.isnan(pos))[0], axis=0)
    neg = np.delete(neg, np.where(np.isnan(neg))[0], axis=0)
    pos=np.delete(pos, np.where(np.isinf(pos))[0], axis=0)
    neg = np.delete(neg, np.where(np.isinf(neg))[0], axis=0)
    ###########
...
def show_performance(pos, neg, method_name='Ours', recall_level=recall_level_default):
    '''
    The input is the probability
    :param pos: 1's class, class to detect, outliers, or wrongly predicted example scores
    :param neg: 0's class scores
    '''

    auroc, aupr, fpr = get_measures(pos[:], neg[:], recall_level)
    print ('{}& {}& {}& {} \\\\'.format(method_name, str(auroc)[0: 4], str(aupr)[0: 4], str(fpr)[0: 4])) 
