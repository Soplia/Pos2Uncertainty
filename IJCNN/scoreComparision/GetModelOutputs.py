...
classNumber = 10
modelName = 'CNN' 
datasetNames = ['CIFAR10', 'SVHN', 'CIFAR100', 'LSUN',
               'KMNIST', 'FMNIST', 'TEXTURE', 'PLACE365'] #  
testLoaders = {'CIFAR10': testLoaderCifar10, 'SVHN': testLoaderSvhn, 'LSUN': testLoaderLsun, 'CIFAR100': testLoaderCifar100,
               'KMNIST': testLoaderKmnist, 'FMNIST': testLoaderFmnist, 'TEXTURE': testLoaderTexture, 'PLACE365': testLoaderplaces365}
device = GetDevice()

model = CifarNN(classNumber)
model.load_state_dict(torch.load('../DictState/Cifar10-CNN.pth'))
ToDevice(model, device)
model.eval()

for datasetName in datasetNames:
    print("{} Testing, model trained by {}...".format(datasetName, modelName))
    ...
    torch.save(outputTensor, '../Outputs/{}-Cifar10CNN-Outputs'.format(datasetName))
