...
oodDatasetNames = ['SVHN', 'CIFAR100', 'LSUN', 
                   'KMNIST', 'FMNIST', 'TEXTURE', 'PLACE365'] # 'CIFAR100', 'LSUN', 'TEXTURE', 'PLACE365', 'KMNIST', 'FMNIST'
scoreTypes = ['Softmax', 'Energy', 'Dirichlet', 'Our']

cifar10Outputs = torch.load('../Outputs/Cifar10-Cifar10CNN-Outputs')
for oodName in oodDatasetNames:
    #print ("Out-of-domain dataset {} is testing...".format(oodName))
    oodOutputs = torch.load('../Outputs/{}-Cifar10CNN-Outputs'.format(oodName))
    print('\multirow{4}{*}{', oodName, '}', end=' ')
    for scoreType in scoreTypes:
        #print("\t {} score is calculating...".format(scoreType))
        inScore = GetScores(cifar10Outputs, method= '{}'.format(scoreType))
        ...
        if oodName == 'SVHN':
            ...
            plt.grid(alpha= 0.8)
            plt.legend()
            #plt.savefig('../Figures/CIFAR10-SVHN-{}'.format(scoreType))
            #plt.show()
            ...
            plt.grid(alpha= 0.8)
            plt.legend()
            #plt.savefig('../Figures/ROC-CIFAR10-SVHN-{}'.format(scoreType))
            plt.show()
